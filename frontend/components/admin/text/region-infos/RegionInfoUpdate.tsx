...
import { getText, updateText } from '@/lib/api/texts'
...
export default function RegionInfoUpdate({ id }: { id: number }) {
  ...
  useEffect(() => {
    Promise.all([getText(id), getRegions()]).then(([info, regions]) => {
      setForm({
        region_id: info.region_id ? String(info.region_id) : '',
        level: info.level ?? 1,
        text: info.text ?? '',
      })
      setRegions(regions)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Nepodařilo se uložit text.')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="flex gap-2">
            {[1, 2, 3].map(lvl => (
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-md font-medium transition-colors border-2 ${
