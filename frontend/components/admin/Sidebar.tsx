...
const getNameAcronym = (firstName?: string, lastName?: string): string => {
    if (!firstName && !lastName) return "U";

    const first = firstName?.trim().charAt(0) || "";
    const last = lastName?.trim().charAt(0) || "";

    const acronym = `${first}${last}`.toUpperCase();
    return acronym || "U";
};
...
export default function Sidebar({onNavigate}: SidebarProps) {
    ...
    const userAcronym = getNameAcronym(user?.first_name, user?.last_name);
    const fullName = user ? `${user.first_name ?? ''} ${user.last_name ?? ''}`.trim() : "Uživatel";
        ...
        <aside className="flex md:w-60 w-40 shrink-0 flex-col border-r border-gray-200 bg-white">
            ...
            <div className="flex items-center justify-between border-t border-gray-200 px-3 py-3">
                <div className="flex items-center gap-2 min-w-0">
                    <div
                        className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-gray-200 bg-gray-100 text-[11px] font-medium text-gray-600 select-none">
                        {userAcronym}
                    </div>
                    <span className="text-xs text-gray-500 truncate">{fullName || "Uživatel"}</span>
                </div>
                <button
