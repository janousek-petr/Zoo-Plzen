...
// Definice dostupných avatarů (v reálu bys tohle mohl mít v databázi)
const unlockedAvatars = [
    "/img/startpage-1.png",
    "/img/startpage-2.png", // Přidej si sem cesty ke svým dalším obrázkům
    "/img/startpage-3.png"
];
...
export default function Settings() {
    ...
    const SettingsRow = ({ label, value, onClick, isLocked = false }: any) => (
        ...
        >
            <span className="cus-text-beige-dark uppercase font-bold text-xl flex items-center gap-2 group-hover:text-white">
                {label} {isLocked && <FaLock size={14} className="opacity-50" />}
            </span>
    ...
    return (
        <main className="py-20 min-h-screen bg-white">
            ...
            {/* AVATAR SEKCE - Kliknutím se otevře výběr */}
            <div 
            ...
            <div className="my-20 px-4">
                <h1 className="uppercase cus-text-beige cus-font-impacted-2 text-6xl text-center mb-10">Nastavení účtu</h1>
            ...
            {editingField && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl border-4 border-[#8B4513]">
                            ...
                            <div className="grid grid-cols-3 gap-4 py-4">
                                {unlockedAvatars.map((img, index) => (
                                    <div 
                                        ...
                                        className={`relative w-20 h-20 cursor-pointer rounded-full overflow-hidden border-4 transition-all ${avatarUrl === img ? 'border-green-500 scale-110' : 'border-transparent hover:border-gray-300'}`}
                                    >
                        ...
                        {editingField === 'name' && (
                            <div className="space-y-4">
                                ...
                                <div className="pt-4 border-t">
                                    ...
                                    <input type="password" value={tempPass} onChange={(e) => setTempPass(e.target.value)} className={`w-full p-3 border-2 rounded-xl ${passError ? 'border-red-500' : ''}`} placeholder="Heslo (heslo123)" />
                                </div>
                            ...
                            <div className="grid gap-3">
                                {['Lehká', 'Střední', 'Těžká'].map((level) => (
                                    <button 
                                        ...
                                        className={`py-4 rounded-xl font-bold text-xl transition-all ${difficulty === level ? 'bg-green-600 text-white' : 'bg-gray-200 hover:bg-gray-300'}`}
                                    >
                                        {level}
                                    </button>
                                ))}
                        ...
                        <button onClick={() => setEditingField(null)} className="mt-6 w-full text-gray-400 font-bold hover:text-gray-600 uppercase text-sm tracking-widest">Zrušit</button>
                    </div>
                </div>
            )}
        </main>
